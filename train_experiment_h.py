"""
Experiment H: Stronger defense reward + goalie positioning, from checkpoint-5122.

Key finding: Agent loses to quick ceia goals (avg 24.5 steps vs 41 for wins).
Fix: Increase defensive penalty 3x + add reward for keeping one teammate
near own goal as a goalie.
"""
import os
import pickle
...
# ── Defense-focused Reward Shaping ──────────────────────────────────────────
class DefenseRewardWrapper(gym.core.Wrapper, MultiAgentEnv):
    """Reward shaping with stronger defense signals."""
    TEAM0_GOAL_X = -13.0
    TEAM1_GOAL_X = 13.0
    ...
    def _shaping(self, info):
        ...
        for aid in info:
            ...
            if self.prev_ball_vel is not None:
                ...
                if delta_speed > 0 and dist < 1.5:
                    shaped[aid] += 0.001 * delta_speed

            # Signal 3: offensive — unchanged
            shaped[aid] += 0.0004 * ball_vel[0] * attack_dir

            # Signal 4: defensive — TRIPLED (key change)
            danger = max(0.0, 1.0 - abs(ball_pos[0] - own_goal_x) / 5.0)
            shaped[aid] -= 0.003 * danger  # was 0.001

            # Signal 5: time penalty — unchanged
            shaped[aid] -= 0.00002
        ...
        # Signal 7: goalie positioning (NEW)
        # Reward one teammate for staying near own goal
        for team_start in (0, 2):
            ids = [team_start, team_start + 1]
            if all("player_info" in info.get(i, {}) for i in ids):
                own_goal_x = self.TEAM0_GOAL_X if team_start == 0 else self.TEAM1_GOAL_X
                pos0 = np.array(info[ids[0]]["player_info"]["position"])
                pos1 = np.array(info[ids[1]]["player_info"]["position"])
                # Which teammate is closer to own goal?
                dist0 = abs(pos0[0] - own_goal_x)
                dist1 = abs(pos1[0] - own_goal_x)
                goalie_id = ids[0] if dist0 < dist1 else ids[1]
                goalie_dist = min(dist0, dist1)
                # Reward for being within 5 units of own goal
                if goalie_dist < 5.0:
                    shaped[goalie_id] += 0.0005 * (1.0 - goalie_dist / 5.0)

        self.prev_ball_vel = ball_vel
        return shaped


def create_defense_env(env_config={}):
    ...
    if "multiagent" in env_config and not env_config["multiagent"]:
        return env
    return DefenseRewardWrapper(env)
# ────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    tune.registry.register_env("Soccer", create_defense_env)
    tmp = create_defense_env()
    ...
    analysis = tune.run(
        ...
        config={
            ...
            # Same optimized hyperparameters as best run (#10)
            "entropy_coeff": 0.005,
            "lambda": 0.95,
            "grad_clip": 0.5,
            "clip_param": 0.2,
