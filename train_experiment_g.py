"""
Experiment G: Near-default PPO hyperparameters + from checkpoint-5122.

Key insight: ceia_baseline was trained with ALL default Ray PPO params.
Our "optimized" hyperparameters may have been hurting, not helping.
Restore defaults except grad_clip=0.5 (prevent catastrophic updates).
"""
import os
import pickle
...
if __name__ == "__main__":
    ...
    analysis = tune.run(
        ...
        config={
            ...
            # ── ALL defaults except grad_clip ────────────────────────────
            # lr=5e-5, entropy_coeff=0.0, lambda=1.0, clip_param=0.3,
            # train_batch_size=4000, sgd_minibatch_size=128, num_sgd_iter=30,
            # vf_loss_coeff=1.0 — exactly how ceia was trained
            "grad_clip": 0.5,              # only non-default: prevent catastrophic updates
            "rollout_fragment_length": 5000,  # same as ceia
