...
dominates the opponent pool but a fixed reference opponent is retained. This 70/30 mix gave us the highest CEIA win
...
residual action error compounds. (b)~\emph{Value-function cold-start}: BC
transfers only the policy network and leaves PPO's critic randomly initialised,
so early advantage estimates are noisy and policy updates are unstable.
...
errors are recoverable. The pattern matches the DAgger
