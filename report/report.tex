...
baseline} and \textbf{100\% (20/20) against a random opponent}. A third agent
...
Win rate against CEIA was measured by 100 evaluation matches
(\texttt{scripts/eval\_vs\_ceia.py}); the random-opponent rate was measured by
20 matches against \texttt{selfmade\_random\_agent}
(\texttt{scripts/eval\_vs\_random.py}). The submitted agent
(\textit{PPO Reward Shaped + Self-Play~70\%~/~CEIA~30\%}, checkpoint-2300) reaches
\textbf{90/100~=~90\%} against CEIA and \textbf{20/20~=~100\%} against the random
...
\textbf{Agent~2: Reward Shaping + 70/30 (\textit{submitted})} & \textbf{20/20 = 100\%} & \textbf{90/100 = 90\%} \\
