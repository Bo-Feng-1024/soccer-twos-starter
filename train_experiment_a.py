"""
Experiment A: High entropy + higher lr from checkpoint-5122 (87%).

Hypothesis: entropy=0.003 is too low, agent stuck in local optimum.
Higher entropy (0.01) + higher lr (1e-4) should allow exploring better tactics.
"""
import os
import pickle
...
def policy_mapping_fn(agent_id, *args, **kwargs):
    ...
    return np.random.choice(
        ...
        p=[0.20, 0.20, 0.60],  # 60% ceia + 40% selfplay
    )
...
if __name__ == "__main__":
    ...
    analysis = tune.run(
        ...
        config={
            ...
            # ── Key changes from train_focused_ceia.py ────────────────────
            "entropy_coeff": 0.01,          # was 0.003 — more exploration
            "lambda": 0.95,
            "grad_clip": 0.5,
            "clip_param": 0.2,
            ...
            "vf_loss_coeff": 0.5,
            "lr_schedule": [
                [33_000_000, 1e-4],        # was 5e-5 — higher lr for exploration
                [45_000_000, 5e-5],
                [60_000_000, 2e-5],
            ],
