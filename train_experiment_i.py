"""
Experiment I: Self-play dominant training (based on Brandão 2022 findings).

Key insight from paper: Training against a fixed opponent alone NEVER improves
the model (Fig 6-8). Self-play creates automatic curriculum learning where
difficulty increases as the agent improves, leading to 93% win rate.

Changes from train_focused_ceia.py:
  - Opponent ratio REVERSED: 30% ceia + 70% self-play (was 70/30)
  - lr = 3e-4 (paper uses 0.0004, was 5e-5)
  - num_sgd_iter = 5 (paper uses 5, was 10)
  - entropy_coeff = 0.005 (more exploration for self-play)
  - Cooldown reduced to 20 (more frequent self-play updates)
"""
import os
import pickle
...
def policy_mapping_fn(agent_id, *args, **kwargs):
    ...
    Team 1 (agents 2, 3) → sampled from opponent pool:
      35% opponent_1 (recent self-play snapshot)
      35% opponent_2 (older self-play snapshot)
      30% opponent_3 (ceia_baseline, fixed)
    """
    if agent_id == 0 or agent_id == 1:
        return "default"
    return np.random.choice(
        ["opponent_1", "opponent_2", "opponent_3"],
        p=[0.35, 0.35, 0.30],  # 70% self-play + 30% ceia (was 30/70)
    )
...
if __name__ == "__main__":
    ...
    analysis = tune.run(
        ...
        config={
            ...
            "vf_loss_coeff": 0.5,
            "lr_schedule": [               # higher lr for stronger updates
                [33_000_000, 3e-4],        # was 5e-5 — paper uses 4e-4
                [50_000_000, 1e-4],
                [80_000_000, 5e-5],
            ],
